...
function BaseNode_References_toString(options) {
    ...
    function dump_reference(follow, reference) {
        ...
        options.add(options.padding + chalk.yellow("               +-> ") + reference.toString(displayOptions) + " " + chalk.cyan(name));
...
function VariableOrVariableType_toString(options) {
    ...
    if (this.nodeClass === node_opcua_data_model_1.NodeClass.Variable) {
        const _dataValue = this._dataValue;
        if (_dataValue) {
            options.add(options.padding +
                chalk.yellow("          value               : ") +
                "\n" +
 ...
 * @private
 */
function _clone_collection_new(newParent, collectionRef, copyAlsoModellingRules, optionalFilter, extraInfo) {
    ...
    for (const reference of collectionRef) {
        ...
        (0, node_opcua_assert_1.assert)(reference.referenceType instanceof node_opcua_nodeid_1.NodeId, "" + reference.referenceType.toString());
        const options = {
            namespace,
...
function _clone_children_references(node, newParent, copyAlsoModellingRules, optionalFilter, extraInfo) {
...
function _clone_non_hierarchical_references(node, newParent, copyAlsoModellingRules, optionalFilter, extraInfo) {
